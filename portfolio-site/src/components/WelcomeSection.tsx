...
const StarsField: React.FC = () => {
  ...
  useEffect(() => {
    // Hanya buat bintang di sisi client untuk menghindari error hydration
    const starElements = Array.from({ length: 100 }).map((_, i) => {
      const size = Math.random() * 3 + 1;
      const left = Math.random() * 100;
      // Bintang di seluruh layar
      ...
      return (
        <div 
          ...
          style={{
            ...
            '--duration': `${duration}s`,
            '--delay': `${delay}s`,
            // Bintang lebih terang di bagian bawah, memudar ke atas
    ...
    <section className="h-screen w-full bg-black flex flex-col justify-center items-center relative">
      ...
      <div className="absolute top-0 left-0 w-full flex items-center justify-between px-8 py-6 z-10">
        ...
        <div className="flex items-center justify-end min-w-[40px]">
          ...
          >
            <svg width="24" height="24" fill="none" stroke="white" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="8" r="1"/><circle cx="12" cy="16" r="1"/><circle cx="12" cy="12" r="1"/></svg>
          </button>
          <AnimatePresence>
            {panelOpen && (
              <motion.div
      ...
      <AnimatePresence>
        {panelOpen && (
          <motion.div
            ...
            className="absolute top-24 right-8 bg-neutral-900 border border-neutral-700 rounded-lg shadow-lg p-4 z-20"
          >
